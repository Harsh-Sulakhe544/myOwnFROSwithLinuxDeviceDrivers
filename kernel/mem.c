...
// KEEP TRACK OF NEXT AND PREV FOR EFFICIENT MANAGEMENET -- DLL DATA STRUCTURE IDEA 
typedef struct dynamic_mem_node {
    uint32_t size;
    bool used; // SIZE USED FOR EACH NODE -- FOR EFFICIENT DELETION
...
void init_dynamic_mem() {
    ...
    dynamic_mem_start->size = DYNAMIC_MEM_TOTAL_SIZE - DYNAMIC_MEM_NODE_SIZE; // 4GB - EACH NODE SIZE 
...
// CHECK USED NODE SIZE AND LEFT OVER 
...
// This function searches for the best-fitting free memory block that can accommodate the requested size.
void *find_best_mem_block(dynamic_mem_node_t *dynamic_mem, size_t size) {
    ...
    while (current_mem_block) {
        ...
        if ((!current_mem_block->used) && (current_mem_block->size >= (size + DYNAMIC_MEM_NODE_SIZE)) &&
...
// This function allocates memory of a specified size.
void *mem_alloc(size_t size) {
    ...
    if (best_mem_block != NULL_POINTER) {
        ...
        dynamic_mem_node_t *mem_node_allocate = (dynamic_mem_node_t *) (((uint8_t *) best_mem_block) + DYNAMIC_MEM_NODE_SIZE +
                                                                        best_mem_block->size);
...
// This function merges the current memory block with the next block if it is free.
...
// This function merges the current memory block with the previous block if it is free.
